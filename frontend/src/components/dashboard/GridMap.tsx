...
export const GridMap: React.FC<GridMapProps> = ({
  ...
  return (
    <section aria-label={title} className="rounded-md border border-slate-200 bg-white p-4 shadow-xs">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-2.5">
        <div>
          ...
          <p className="text-xs text-slate-500">
            <span className="font-semibold text-emerald-700">{counts.online} Nominal</span>
            {' | '}
            <span className="font-semibold text-amber-700">{counts.warning} Constrained</span>
            {' | '}
            <span className="font-semibold text-red-700">{counts.offline} Outage/Faulted</span>
          </p>
        </div>
        {/* Status Legend */}
        <div className="flex flex-wrap gap-2 text-[11px] text-slate-700" aria-label="Status legend">
          {(['online', 'warning', 'offline'] as const).map((s) => (
            <span key={s} className="flex items-center gap-1.5 rounded border border-slate-200 bg-slate-50 px-2 py-0.5 font-bold uppercase">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_260px]">
        ...
        <div className="overflow-hidden rounded-md border border-slate-200 bg-[#F8FAFC]">
          <svg viewBox="0 0 100 62" className="h-auto w-full" role="img" aria-label="Microgrid single-line diagram">
              ...
              return (
                <g key={e.id}>
                  <line
                    ...
                    x2={b.x}
                    y2={b.y}
                    stroke={dead ? '#DC2626' : isWarning ? '#D97706' : '#16A34A'}
            ...
            {nodes.map((n) => (
              ...
              >
                ...
                {/* Kind Glyph Tag */}
                <text x={n.x} y={n.y + 0.3} textAnchor="middle" fontSize={1.8} dominantBaseline="central" fill="#1E293B" fontWeight={800} pointerEvents="none">
                  {KIND_GLYPH[n.kind]}
                </text>
                {/* Node ID Tag */}
                <text x={n.x} y={n.y + 5.2} textAnchor="middle" fontSize={1.8} fill="#0F172A" fontWeight={700} pointerEvents="none">
                  {n.id}
                </text>
              </g>
            ))}
        ...
        <aside className="rounded-md border border-slate-200 bg-white p-3.5 shadow-xs" aria-live="polite">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">SCADA Inspector</p>
              <h4 className="text-sm font-extrabold text-slate-900">{selected.label}</h4>
            </div>
            <span className="font-mono text-xs font-bold text-slate-600">{selected.id}</span>
          </div>

          <div className="mt-2.5 flex items-center justify-between">
            <span className="text-xs text-slate-500">Kind: <strong className="text-slate-800 uppercase">{selected.kind}</strong></span>
            <span
          ...
          <dl className="mt-3 space-y-1.5 font-mono text-xs">
            <div className="flex justify-between rounded border border-slate-200 bg-slate-50 px-2 py-1">
              <dt className="text-slate-500">Active Load</dt>
              <dd className="font-extrabold text-slate-900">{selected.loadKw.toFixed(1)} / {selected.capacityKw.toFixed(0)} kW</dd>
            </div>
            <div className="flex justify-between rounded border border-slate-200 bg-slate-50 px-2 py-1">
              <dt className="text-slate-500">Load Factor</dt>
              <dd className="font-extrabold text-slate-900">{Math.round((Math.abs(selected.loadKw) / selected.capacityKw) * 100)}%</dd>
            </div>
            <div className="flex justify-between rounded border border-slate-200 bg-slate-50 px-2 py-1">
              <dt className="text-slate-500">Nominal Voltage</dt>
              <dd className="font-extrabold text-slate-900">{selected.voltageKv === 0 ? '415 V' : `${selected.voltageKv} kV`}</dd>
            </div>
            <div className="flex justify-between rounded border border-slate-200 bg-slate-50 px-2 py-1">
              <dt className="text-slate-500">Grid Frequency</dt>
              <dd className="font-extrabold text-emerald-700">50.00 Hz</dd>
            </div>
          </dl>

          {/* Utilization progress bar */}
          <div className="mt-2.5 h-1.5 overflow-hidden rounded bg-slate-100" role="progressbar" aria-valuenow={Math.round((Math.abs(selected.loadKw) / selected.capacityKw) * 100)} aria-valuemin={0} aria-valuemax={100} aria-label={`${selected.label} utilization`}>
            <div
          ...
          <p className="mt-2.5 text-xs text-slate-600">{selected.note}</p>

          {/* Manual Operator Controls */}
          <div className="mt-3 pt-2.5 border-t border-slate-100 flex flex-col gap-1.5">
            <span className="text-[10px] font-bold uppercase text-slate-500">Operator Override Controls</span>
            <div className="grid grid-cols-2 gap-1.5">
              <button
                onClick={() => alert(`Isolating Node ${selected.id} - Breaker Tripped (SCADA Guarded)`)}
                className="flex items-center justify-center gap-1 rounded border border-red-300 bg-red-50 px-2 py-1 text-xs font-bold text-red-700 hover:bg-red-100"
              >
                <Power size={12} /> Isolate Node
              </button>
              <button
                onClick={() => alert(`Set setpoint for ${selected.id} dispatched via API`)}
                className="flex items-center justify-center gap-1 rounded border border-slate-300 bg-slate-50 px-2 py-1 text-xs font-bold text-slate-700 hover:bg-slate-100"
              >
